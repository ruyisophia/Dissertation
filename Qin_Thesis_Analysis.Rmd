...
setwd("/Users/ruyi/Dropbox/Dissertation/Analysis/R")
getwd()
...
```{r, include=FALSE}
# visualize individual variation in PC1 by register

pc1.register = ddply(dat, # input dataframe
    .(sid),  # group by student
    summarize,  # create new columns as follows: 
    Academic=mean(pc1[register=="academic"], na.rm=TRUE),
    Colloquial=mean(pc1 [register=="colloquial"], na.rm=TRUE)
    )
pc1.register

ggplot(pc1.register, 
    aes(x=Academic,   # put Academic PC1s on x-axis
    y=Colloquial, # and Colloquial PC1s on y-axis
    )) +      
    geom_point(aes(color="empirical proportion")) +  # make a scatter-plot
    xlim(-4.5,15.5) + # make the limits for the x-axis 
    ylim(-4.5,15.5) + # and y-axis the same
    geom_abline(
        intercept=0, 
        slope=1,
        ) +
...
###END OF THE DAY
# create a data frame with one row per individual
student.native = ddply(dat, 
    .(sid),    # group by student
    summarize,    # create columns as follows:
    native=native[1], # one column for type
    # and one column for standardized mean(belong):
    l_token=l_token[1],
    program_m=program_m[1],
    efset_std=efset_std[1]
    )

# first make a dataframe of academic
new.aca = student.native
new.aca["register"]="academic"
# then a dataframe of colloquial
new.col = student.native
new.col["register"]="colloquial"
# then merge them
new.data = rbind(new.aca, new.col)
head(new.data)


# generate predictions using the PC1 Model:
new.data["pc1.pred"] = predict(
    LS.M2,        # PC1 Model
    newdata=new.data, # to generate predictions for
    type="response")      # predicted pc1

# generate predictions using the model with level-2 covariates
new.students["level_2"] = predict(
    mcc.q3.both, 
    newdata=new.students, # same students
    type="response")      # probabilities
# make a separate row for each model
new.gathered = gather(new.students, 
    "model", # key
    "pred",  # value
    rand_slope, # the two columns...
    level_2     # to be turned into one
    )
...
calc = function(efset, Fr, Sp, Aca, LS.M6, u=0) {
  lexsyn_pc3 = fe[[1]] + fe[[2]]*efset + fe[[3]]*Aca + fe[[4]]*Fr + fe[[5]]*Sp + 
    fe[[6]]*184 + fe[[7]]*2 + fe[[8]]*Aca*Fr + fe[[9]]*Aca*Sp +u
}
...
DM.M1 <- glmer(d_org ~ factor(register) + (1|sid) + (1|tid),
          family=poisson,
          offset=log(l_token),
          data=dat)
summary(DM.M1)
...
DM.M13 <- glmer(d_sta ~ factor(register) + (1|sid) + (1|tid),
          family=poisson,
          offset=log(l_token),
          data=dat)
summary(DM.M13)
...
#Write the function
calc.probs = function(efset, Fr, Sp, Aca, DM.M12, u=0) {
  eng = fe[[1]] + fe[[2]]*Aca + fe[[3]]*efset + fe[[4]]*Fr + fe[[5]]*Sp + 
...
#Set up the dataframe for plotting
Col.Fr=calc.probs(efset, 1, 0, 0, DM.M12, 0)
Col.Fr=data.frame(register="colloquial", efset_std=efset, eng=Col.Fr, native="French")
Col.Sp=calc.probs(efset, 0, 1, 0, DM.M12, 0)
Col.Sp=data.frame(register="colloquial", efset_std=efset, eng=Col.Sp, native="Spanish")
Col.Ch=calc.probs(efset, 0, 0, 0, DM.M12, 0)
Col.Ch=data.frame(register="colloquial", efset_std=efset, eng=Col.Ch, native="Chinese")

Aca.Fr=calc.probs(efset, 1, 0, 1, DM.M12, 0)
Aca.Fr=data.frame(register="academic", efset_std=efset, eng=Aca.Fr, native="French")
Aca.Sp=calc.probs(efset, 0, 1, 1, DM.M12, 0)
Aca.Sp=data.frame(register="academic", efset_std=efset, eng=Aca.Sp, native="Spanish")
Aca.Ch=calc.probs(efset, 0, 0, 1, DM.M12, 0)
Aca.Ch=data.frame(register="academic", efset_std=efset, eng=Aca.Ch, native="Chinese")

median.eng=rbind(Col.Fr, Col.Sp, Col.Ch, Aca.Fr, Aca.Sp, Aca.Ch)

#Visualize the model
P.DM.M12 <-ggplot(data=median.eng, aes(x=efset_std, y=eng, col=native)) +
  geom_line(aes(linetype=register)) +
  scale_linetype_manual(breaks=c("academic", "colloquial"), values=c(2, 1)) +
  xlab("Standardized L2 Proficiency") +
  ylab("Engagement Markers (Probability)") 
